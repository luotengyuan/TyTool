...
// ARM64's arm_neon.h has already defined vdivq_f32_ vsqrtq_f32_.
#if !defined (WEBRTC_ARCH_ARM64)
static float32x4_t vdivq_f32_(float32x4_t a, float32x4_t b) {
  int i;
...
static float32x4_t vsqrtq_f32_(float32x4_t s) {
  int i;
...
static void ScaleErrorSignalNEON(AecCore* aec, float ef[2][PART_LEN1]) {
  ...
  for (i = 0; i + 3 < PART_LEN1; i += 4) {
    ...
    float32x4_t ef_re = vdivq_f32_(ef_re_base, xPowPlus);
    float32x4_t ef_im = vdivq_f32_(ef_im_base, xPowPlus);
    const float32x4_t ef_re2 = vmulq_f32(ef_re, ef_re);
    const float32x4_t ef_sum2 = vmlaq_f32(ef_re2, ef_im, ef_im);
    const float32x4_t absEf = vsqrtq_f32_(ef_sum2);
    const uint32x4_t bigger = vcgtq_f32(absEf, kThresh);
    const float32x4_t absEfPlus = vaddq_f32(absEf, k1e_10f);
    const float32x4_t absEfInv = vdivq_f32_(kThresh, absEfPlus);
...
static void SubbandCoherenceNEON(AecCore* aec,
...

  {
    ...
    for (i = 0; i + 3 < PART_LEN1; i += 4) {
      ...
      vec_cohde = vdivq_f32_(vec_cohde, vec_sdse);
      vec_cohxd = vmlaq_f32(vec_cohxd, vec_sxd.val[1], vec_sxd.val[1]);
      vec_cohxd = vdivq_f32_(vec_cohxd, vec_sdsx);

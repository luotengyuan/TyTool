...
public class BytesTransUtil {
    ...
    /****************************************/
    public short[] Bytes2Shorts(byte[] buf, int len) {
        byte bLength = 2;
        if (buf == null){
            return null;
        }
        len = buf.length < len ? buf.length : len;
        short[] s = new short[len / bLength];
    ...
    public short[] Bytes2Shorts(byte[] buf) {
        return Bytes2Shorts(buf, buf == null ? 0 : buf.length);
    }

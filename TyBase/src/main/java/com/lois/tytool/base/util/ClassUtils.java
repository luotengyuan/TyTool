...
import com.lois.tytool.base.constant.BaseTypeConstants;
			...
			try {
				...
				while (dirs.hasMoreElements()) {
					...
					if (FileConstants.FILE_PROTOCOL.equals(protocol)) {
						...
						com.lois.tytool.base.debug.TyLog.v("扫描的文件路径filePath: {}", filePath);
						...
						try {
									...
									if ((idx != -1) || recursive) {
										...
										if (name.endsWith(FileConstants.CLASS_FILE) && !entry.isDirectory()) {
											...
											try {
												...
												com.lois.tytool.base.debug.TyLog.e("找不到指定class：{}",packageName + DOT + className, e);
											}
										}
									}
									}
							...
							com.lois.tytool.base.debug.TyLog.e("从jar包获取文件出错", e);
						} finally {
							if (jar != null) {
								jar.close();
							}
		...
		for (File file : dirfiles) {
			...
			if (file.isDirectory()) {
				...
				try {
					...
					com.lois.tytool.base.debug.TyLog.e("找不到指定class：{}", packageName + '.' + className, e);
				}
			}
		}
	}
